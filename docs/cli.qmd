...
Format a Quarto, Pandoc, or Markdown document according to panache's formatting rules. By default, outputs the formatted content to stdout. Use --write to format in place or --check to verify formatting without making changes.
...
* `<FILE>` — Path to the input file to format. If not provided, reads from stdin. Supports .qmd, .md, .Rmd, and other Markdown-based formats.
...
* `--write` — Write the formatted output back to the input file, modifying it in place. Cannot be used with stdin input. It's recommended to use version control before using this option.
