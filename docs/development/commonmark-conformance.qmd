...
description: >-
  How Panache's shared parser scores against the CommonMark spec test suite
  under the CommonMark flavor, and where it currently diverges.
---
...
Inputs.table(
    data.sections.map((s) => ({
        ...
        "Pass %":
            s.pass + s.fail === 0
                ? ""
                : `${((100 * s.pass) / (s.pass + s.fail)).toFixed(0)} %`,
    })),
    {
        sort: "Section",
