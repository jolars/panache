...
panache can invoke external formatters for code blocks. Formatters run in
parallel for maximum performance.

### Built-in Presets

panache ships with sensible defaults for common languages—**zero configuration
required!**

**Default formatters (automatically enabled):**

- **R**: `air format` - [air](https://github.com/etiennebacher/air) R formatter
- **Python**: `ruff format` - [Ruff](https://github.com/astral-sh/ruff) Python
  formatter

If these tools are installed, your R and Python code blocks will be
automatically formatted. If not, the original code is preserved with no errors.

**Using preset formatters:**

```toml
# Option 1: Use a different preset for R
[formatters.r]
preset = "styler"  # Available: "air" (default), "styler"

# Option 2: Switch Python formatter
[formatters.python]
preset = "black"   # Available: "ruff" (default), "black"
```
...
| -------- | -------- | -------------------------- | ------------------------- |
| `r`      | `air`    | `air format {}`            | Default, fast             |
| `r`      | `styler` | `Rscript -e "styler::..."` | Requires styler R package |
| `python` | `ruff`   | `ruff format`              | Default, fast             |
| `python` | `black`  | `black -`                  | Popular alternative       |

**Disabling built-in defaults:**
...
[formatters.r]
preset = "styler"  # Switch from default "air"

# Option 2: Full custom configuration (overrides default)
[formatters.python]
cmd = "black"
args = ["-", "--line-length=88"]
