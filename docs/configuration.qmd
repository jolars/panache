...
`exclude`
:   Base exclude patterns. If set, replaces Panache's default exclude base.
...
### Code Block Style (Deprecated)

`[format.code-blocks]` and top-level `[code-blocks]` are deprecated and now
no-ops. If explicitly set, panache prints a warning during config loading.
