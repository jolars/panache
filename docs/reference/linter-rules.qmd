...
flagging unknown/misspelled keys and type mismatches.
...
Diagnostic codes
:   [`quarto-schema-unknown-key`](#quarto-schema),
...
:   Unlike pandoc---where metadata is an arbitrary mapping---Quarto ships a
    ...
    - **Unknown keys.** On *closed* schema objects (e.g. a specific format's
      option block) any undeclared key is flagged. On the *open* top level
      (which must allow pandoc passthrough and custom metadata) only a near-miss
      typo of a known key is flagged, with a "did you mean" suggestion.
