...
:   The math parser captures math content losslessly even when it is malformed,
    ...
    formatting --- but it usually means the equation will not render as the
    author intended. This rule surfaces the structural problems the parser
    already detected, with the diagnostic pointing at the offending byte (the
    unclosed `{`, the stray `}`, or the mismatched `\end`).
...
```
warning[math-unclosed-group]: unclosed `{` group
 --> document.qmd:1:38
  |
