...
(e.g. `[https://example.com/](https://example.com/)`)---typically an artifact of
HTML→Markdown conversion---and offers to rewrite them as an autolink.
...
:   Pandoc footnotes do not nest. Inside a `[^x]: ...` definition body, any
    `[^id]` reference is silently parsed as a literal `Str`---the would-be link
    disappears from the output with no warning. The same applies to references
    nested arbitrarily deep inside that body (inside emphasis, strong,
    strikeout, links, blockquotes, lists, or inline footnotes).
