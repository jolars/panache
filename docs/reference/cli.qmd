...
* `-q`, `--quiet` — Suppress informational status messages on stdout (e.g. "Formatted X", "N file left unchanged", "All files are correctly formatted", "No issues found"). Errors are still written to stderr, and primary command output (such as formatted content when reading from stdin, lint diagnostics, JSON/Markdown reports, or the parsed CST) continues to print so that pipelines keep working.
