...
* `--blank-lines <MODE>` — Override how blank lines are handled. Takes precedence over `[format] blank-lines` in panache.toml. Options: `collapse` (collapse multiple blank lines into one, the default), `preserve` (keep all existing blank lines).
