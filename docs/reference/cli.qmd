...
* `--no-cache` — Disable all lint/format cache reads and writes for this run. Can also be enabled with RUFF_NO_CACHE.
* `--cache-dir <CACHE_DIR>` — Path to the cache directory for this invocation. Overrides config `cache-dir`. Can also be set with RUFF_CACHE_DIR.
