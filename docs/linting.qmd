...
`panache format --check`). It primarily tries to catch
...
Panache includes several built-in lint rules that analyze document structure
and content.
...
Description
:   Each reference label and footnote ID must be unique within a document. Duplicate
    definitions cause ambiguity—only the first definition is used, making the others
    ineffective.
...
:   Checks that all cited keys (`[@key]`) exist in the configured bibliography
    files. Also validates inline bibliography entries for duplicates and conflicts.
...
The parser emits syntax errors as lint diagnostics when it encounters malformed
input.
...
Description
:   Parser errors indicate syntactically invalid Markdown that panache cannot parse
    correctly. These should be fixed manually as they typically indicate incomplete
...
**Resolution:** Fix the syntax to match valid Markdown/Pandoc specifications.
...
correctly understands that `x` is defined before it's used.
...
Lint rules can be configured in the `[lint]` section of your configuration file
(planned feature):
...
# duplicate-references = true
```

::: {.callout-note}
Per-rule configuration is a planned feature. Currently, all rules are enabled
by default.
:::
