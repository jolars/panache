...
```toml
cache-dir = ".panache-cache"
```

`cache-dir`
:   Optional cache directory override. Relative paths are resolved from the CLI
    start directory. If unset, Panache uses a project-local `.panache-cache/`.
...
`--cache-dir <CACHE_DIR>` (or `RUFF_CACHE_DIR`) to override `cache-dir` for one
...
| C/C++     | `astyle`        | `astyle --quiet`                        | Stdin      |
| Assembly  | `asmfmt`        | `asmfmt`                                | Stdin      |
| Text/CJK  | `autocorrect`   | `autocorrect --stdin`                   | Stdin      |
| CMake     | `cmake-format`  | `cmake-format -`                        | Stdin      |
| CUE       | `cue-fmt`       | `cue fmt -`                             | Stdin      |
| Jsonnet   | `jsonnetfmt`    | `jsonnetfmt -`                          | Stdin      |
| Markdown  | `mdformat`      | `mdformat -`                            | Stdin      |
| SQL       | `sqlfmt`        | `sqlfmt -`                              | Stdin      |
| Nix       | `alejandra`     | `alejandra`                             | Stdin      |
| Nix       | `nixfmt`        | `nixfmt`                                | Stdin      |
| Go        | `gofmt`         | `gofmt`                                 | Stdin      |
| Go        | `gofumpt`       | `gofumpt`                               | Stdin      |
| Gleam     | `gleam`         | `gleam format --stdin`                  | Stdin      |
| Terraform | `terraform-fmt` | `terraform fmt -no-color -`             | Stdin      |
| YAML      | `yamlfix`       | `yamlfix -`                             | Stdin      |
| YAML      | `yq`            | `yq -P -`                               | Stdin      |
