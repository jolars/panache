...
hashpipe (`#|`) chunk options. Plain scalars reflow to `line-width`, and
*folded* block scalars (those introduced with `>`, including the `>-` and `>+`
chomping variants) have their overlong lines reflowed too, since a folded
scalar's single line breaks fold to spaces. The continuation lines keep the
scalar's indentation, and short lines are never joined.

Literal block scalars (`|`) are left untouched --- their line breaks are
significant content --- as are quoted scalars. Under `wrap = "preserve"`,
nothing in the YAML is reflowed.
...
Only structurally-safe transforms run; operator spacing and macro contents are
never rewritten. Alignment uses **source character widths** (it tidies the
source, it does not align rendered glyphs), and malformed math is left
untouched. The option is unstable and may change without a major release --- see
