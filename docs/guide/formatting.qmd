...
plus stays tight (`x = -y`, `f(-x)`). Macro contents are not rewritten (no
`\frac` canonicalization, no auto-`&` insertion). Alignment uses **source
character widths** (it tidies the source, it does not align rendered glyphs),
and malformed math is left untouched. The option is unstable and may change
