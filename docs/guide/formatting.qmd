...
relations, with continuations aligned under the first relation:
...
which stay with their terms; operators inside `(…)`, `\left…\right`, or a `{…}`
group (including `\frac{…}{…}` arguments) are never broken at. An equation with
no top-level relation is left on one line even when it overflows. The option is
