...
`line-ending`, `line-width`, `pandoc-compat`, `format`, ...
```
...
Only structurally-safe transforms are applied --- never operator spacing,
`\frac`/`\dfrac` rewriting, or inserting alignment `&`. Column alignment uses
**source character widths**, so it tidies the source rather than aligning
rendered glyphs (`\alpha` counts as six columns). Malformed math (unbalanced
braces or environments) and content with a lone unescaped `$` is left verbatim.
...
:   Suppress the soft-break-derived space when a newline sits between two East
    ...
    disabled, non-default extension. No effect when `wrap = "preserve"`, since
    the newline is kept verbatim.
