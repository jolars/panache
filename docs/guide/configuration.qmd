...
convention) keeps the project root tidy. A `.config/panache.toml` is treated
like any config that lives in a subdirectory: most settings are unaffected, but
**path-based** settings --- `exclude` / `include` and `flavor-overrides` glob
keys --- resolve relative to the `.config/` directory, not the directory above
it (see [Path matching base](#file-selection-directory-traversal)). To target
files outside `.config/`, write the pattern accordingly (for example
`../vendor/**`).
...
- For discovered config files, include/exclude globs are resolved relative to
  the directory containing the active config file --- including a
  `.config/panache.toml`, whose globs resolve relative to `.config/`.
