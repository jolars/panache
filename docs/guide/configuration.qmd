...
:   Optional cache directory override. Relative paths are resolved from the CLI
    ...
    (for example `~/.cache/panache/cli-cache/` on Linux).
...
reads/writes for a single CLI invocation. Use `panache cache clean` to remove
the current workspace bucket, or `panache cache clean --all` to clear all global
Panache CLI cache buckets.
