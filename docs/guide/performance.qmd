...
description: >
  ...
  and mdformat on real Quarto and Markdown documents. The benchmarks highlight
...
rumdl, and mdformat on a realistic corpus of Quarto and Markdown documents. We
don't include R Markdown benchmarks here because no other tool supports that
format.
...
html`<div class="callout-note">
  ...
  <ul style="margin: 0">
    ...
    <li>panache ${data.meta.tools.panache.version}, prettier ${data.meta.tools.prettier?.version ?? "n/a"}, pandoc ${data.meta.tools.pandoc?.version ?? "n/a"}, rumdl ${data.meta.tools.rumdl?.version ?? "n/a"}, mdformat ${data.meta.tools.mdformat?.version ?? "n/a"}</li>
  </ul>
</div>`
```
