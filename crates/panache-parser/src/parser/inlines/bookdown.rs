...
pub fn try_parse_bookdown_reference(text: &str) -> Option<(usize, &str)> {
...
pub fn try_parse_bookdown_definition(text: &str) -> Option<(usize, &str)> {
    let bytes = text.as_bytes();
...
pub fn try_parse_bookdown_text_reference(text: &str) -> Option<(usize, &str)> {
    let bytes = text.as_bytes();
    if !bytes.starts_with(b"(ref:") {
        return None;
    }
...
pub fn try_parse_bookdown_equation_definition(text: &str) -> Option<(usize, &str)> {
